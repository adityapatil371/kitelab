...
report files (removed 2026-08-28 at Aditya's request):
    ...
    BTC Validation     -- preserved copy of the manual-backtest validation. The
                          only unreproducible content in any report: no script
                          can regenerate it. Source of truth is the Google Sheet
                          "Aditya P bitcoin manual"; export it to
                          output/Aditya P bitcoin - validated.xlsx. Missing is a
...
def require_btc_validation() -> None:
    """Stop before any work if the hand-made BTC sheet is absent.

    It is the only unreproducible content in any report: no script can rebuild it.
    A bare `if exists()` used to skip it in silence, so a master report could ship
    without it and nothing said so (it did, on 2026-08-31).
    """
    ...
    raise SystemExit(
        ...
        "  Refusing to write a master report without it.")


def main() -> None:
    require_btc_validation()          # 15 minutes of work is not worth a silent gap
    ...
    # ------------------------------------------------------ BTC validation ----
    # This sheet is the ONLY unreproducible content in any report: a hand-made
    # backtest that no script can regenerate. A bare `if exists()` used to skip it
    # in silence, so a master report could ship without it and nothing said so
    # (it did, on 2026-08-31). Missing is now a hard stop, with the recovery path.
